...
const PieChart = props => {
  ...
  const initData = () => {
    ...
    let options = {
      ...
      tooltip: {
        ...
        position: 'top',
        extraCssText:
          'background:linear-gradient(135deg, #25C1E7 0%, rgba(0,210,255,0.00) 100%);',
        textStyle: {
          fontSize: 16,
        },
      },
      color:
        !!props.pieColor && props.pieColor.length !== 0
          ? props.pieColor
          : ['#F16317', '#FFC21C', '#1FDBFA', '#ffdf6f', '#968ade'], //手动设置每个图例的颜色
      legend: {
        //图例组件
        show: true,
        ...
        formatter: params => {
          ...
          return (
            params +
            '          ' +
            data[legendIndex].value +
            '  ' +
            parseInt((data[legendIndex].value / total) * 100, 10) +
            '%'
          );
        },
      },
  ...
  return <div id={props.siteId} style={props.style}></div>;
};
